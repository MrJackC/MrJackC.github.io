---
aliases: '调试排错 - Java 线程分析之线程Dump分析'
tags: 
cssclass:
source:
order: 240
...
![image-20220825223611520](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202403121204783.png)
...
- **查找占用CPU最多的线程**
  3. 使用命令：top -H -p pid（pid为被测系统的进程号），找到导致CPU高的线程ID，对应thread dump信息中线程的nid，只不过一个是十进制，一个是十六进制；
  4. 在thread dump中，根据top命令查找的线程id，查找对应的线程堆栈信息；
- **CPU使用率不高但是响应很慢**
