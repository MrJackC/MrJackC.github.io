...
2. 在main方法中：
   ...
   2）、 submit.doSubmit(args) -->执行提交程序,点击doSubmit
         ...
         ②、appArgs.action match {case SparkSubmitAction.SUBMIT => submit(appArgs, uninitLog)-->点击submit
                submit中又调用了doRunMain()，doRunMain()中调用了runMain()方法
...
<img src="https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281242377.png" alt="image-20200618002757465" style="zoom:50%;" />

<img src="https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281245563.png" alt="image-20200618002929263" style="zoom:50%;" />

![image-20200619202433592](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281242225.png)

#### 2.2.4 CoarseGrainedExecutorBackend

> 执行一次- bin/java org.apache.spark.executor.YarnCoarseGrainedExecutorBackend这个命令 就会执行一个新的进程，则是属于并行执行的感觉，和之前执行的内容是分开的。类似我们在Windows中开了一个微信和qq程序一样，各自执行，互不影响。
...
![image-20200618150421861](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281242576.png)

![image-20200618150442390](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281245545.png)
...
<img src="https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281242134.png" alt="image-20200618155642818" style="zoom:150%;" />
...
2. 通信环境：NettyRpcEnvFactory() 创建环境工厂 创建环境
...
![image-20200618201029135](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281242943.png)
...
![image-20200618202126937](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281246471.png)
...
![image-20200618203801740](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281242980.png)
...
3. SparkContext，是spark核心的对象，核心类，在这个核心类中的一些重要的参数有：
 ...
 -- _conf：下滑线开头，表示内部的变量，不是规范，是早期C程序员默认遵守的规范。
```
...
![image-20200618211748700](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281243911.png)
...
```sql
 调用行动算子后，触发作业的执行过程：
	...
	6. 在eventQueue里面有一个线程，线程中有一个run方法,在这个线程进行死循环(不会阻塞到主线程),不断取出消息
	--> eventThread.start()
	7.  子线程的run中负责取出消息，因为这个队列是一个阻塞式队列，队列中没有消息，那么就处于阻塞式状态
	--> val event = eventQueue.take() 
	8. 取到消息
	--> onReceive(event)
	9. 执行处理消息
	--> doOnReceive(event)
	10. 通过使用模式匹配的的方式处理消息(JobSubmitted事件会被下面匹配到)
	-->  def doOnReceive(event: DAGSchedulerEvent): Unit = event match {
      		-- case JobSubmitted(jobId, rdd, func, partitions, callSite, listener, properties) 
        11. 有向无环图调度器处理任务的提交 (JobSubmitted事件会被下面匹配到执行下面的操作)
        --> dagScheduler.handleJobSubmitted(jobId, rdd, func, partitions, callSite, listener, properties)
        	涉及到stage的划分
        	 11.1创建stage(最后的一个result stage)
		--finalStage = createResultStage(finalRDD, func, partitions, jobId, callSite)  //resultstage和finalstage是一个东西
		11.2每个shuffle依赖创建一个ShuffleMapStage
		-- getOrCreateParentStages
		    -- getShuffleDependencies(rdd).map { shuffleDep =>getOrCreateShuffleMapStage(shuffleDep, firstJobId) }.toListshufflestage
...
![image-20200620130734058](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281243639.png)
...
![image-20200621180817513](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281243888.png)



![image-20200620004312766](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281243667.png)
...
![image-20200620012427321](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281243978.png)

- 堆外内存

![image-20200620015214858](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281243782.png)
...
![image-20200620015026859](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281244514.png)

- 堆外内存

![image-20200620015251558](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281244068.png)
...
![image-20200620015447725](https://cdn.jsdelivr.net/gh/MrJackC/PicGoImages/other/202503281245361.png)
